...
function reverseUsingGeneric<T>(item:T): string | number{
    if(item === null || item === undefined){
        return ('item type should not empty / to be string or number');
    }

    const str = item.toString();
    const result = str.split('').reverse().join('');
    return typeof item === 'number' ? Number(result): result
}

console.log(reverseUsingGeneric('Anand'));


function reverseStringNumber(item:unknown): string | number {
    if(item === null || item === undefined){
        return ('item type should not empty / to be string or number');
    } else{
...
console.log(reverseStringNumber('Bangalore'));

function reverseNumberUsingWhileForLoop(): void{
    let num: number = 1234;
    let temp:number = num;
    let rev : number = 0;
...
reverseNumberUsingWhileForLoop();
...
function findIntersection(arr1: number[], arr2: number[]) {
    ...
    let newArr1: Set<number> = new Set(arr1);
    let newArr2: number[] = [...new Set(arr2)];
    let interseaction: number[] = newArr2.filter(item => newArr1.has(item));
...
function sortStringArrayAscDescOrder(){
    const items = ['kolkata','Delhi','Agra','Goa'];

    const sortStringArrayAscOrder: string[] = items.sort((a,b) => a.localeCompare(b));

    console.log(`Sort string array in ascending order result: ${sortStringArrayAscOrder}`);
    const sortStringArrayDescOrder: string[] = items.sort((a,b) => b.localeCompare(a));

    console.log(`Sort string array in descending order result: ${sortStringArrayDescOrder}`);
}
sortStringArrayAscDescOrder();

function sortNumberArrayAscDescOrder(){
    let item: number[] = [43,24,11,52,1,34,24];

    // @ts-ignore
    const ascOrder: number[] = [... new Set(item)].sort((a: number,b: number) => a-b);
    console.log(` Ascending order result: ${ascOrder}`);

    // @ts-ignore
    const descOrder: number[] = [... new Set(item)].sort((a: number,b: number) => b-a);
    console.log(` Descending order result: ${descOrder}`);
}