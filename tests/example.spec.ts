...
test.describe("Automation practice ", () => {
  test(
    ...
    async ({ page, automationPractice }) => {
      await expect(automationPractice).toHaveTitle(
  ...
  test("Handle new tab", async ({ context, page }) => {
    ...
    const [newTab] = await Promise.all([
      ...
      page
        .locator('button[onClick="myFunction()"]')
  ...
  test("handle simple popup", async ({ page, automationPractice }) => {
    ...
    automationPractice.on("dialog", async (dialog) => {
      console.log("simple alert msg", dialog.message());
      await dialog.accept();
    });

    await automationPractice.click("#alertBtn");
    const simpleAlertMsg = await automationPractice
      .locator("#demo")
      .textContent();
  });
  ...
  test("Handle pop up window", async ({
    context,
    page,
    automationPractice,
  }) => {
  ...
  test("Use fixture", { tag: ["@orangehrm"] }, async ({ loggedInPage }) => {
